...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Plot bead maps! Usage: plot_beadmaps.py -s 2IDO -e 9. "
        "Flag -h for more details.")
...
def get_xlinked_residues(xlink_file, proteins_optimized):
    ...
    for ln in xlf.readlines():
        ...
        # TODO for multiple proteins ideally xlinked_res is a dictionary
        # keyed by protein name!
        if prot1 in proteins_optimized and int(res1) not in xlinked_res:
            xlinked_res.append(int(res1))
        if prot2 in proteins_optimized and int(res2) not in xlinked_res:
...
def generate_precisions_per_residue(precision_file, beadmap_file,
                                    proteins_optimized):

    beadwise_precisions = get_beadwise_precisions(precision_file,
                                                  proteins_optimized)
    ...
    precision_per_residue = numpy.expand_dims(
        numpy.array(precision_per_residue), axis=0)
...
def plot_beadmaps():
    ...
    data_dir = os.path.join(os.path.expanduser('~'), "optrep",
                            "expts/expt"+arg.experiment, arg.system)

    config_file = os.path.join(os.path.expanduser('~'), "optrep", "input",
                               arg.system,
                               arg.system+".config."+arg.experiment)

    config_params = stats_helper.parse_config_file(config_file)

    xlinked_residues = get_xlinked_residues(
        os.path.join(os.path.expanduser('~'), "optrep", "input",
                     arg.system, config_params["XLINKS_FILE"][0]),
        config_params["PROTEINS_TO_OPTIMIZE_LIST"])
    ...
    for ires, resolution in enumerate(config_params["RESOLUTIONS_LIST"]):
        ...
        precision_file = os.path.join(data_dir, "r"+resolution,
                                      "bead_precisions_"+resolution+".txt")

        beadmap_file = os.path.join(data_dir, "r"+resolution,
                                    "bead_map_"+resolution+".txt")

        (precision_residues[resolution],
         residue_range, bead_boxes[resolution]) = \
            generate_precisions_per_residue(
                precision_file, beadmap_file,
                config_params["PROTEINS_TO_OPTIMIZE_LIST"])

        min_precision = min(min_precision,
                            min(precision_residues[resolution][0]))
        max_precision = max(max_precision,
                            max(precision_residues[resolution][0]))
    ...
    ax3.imshow(
        precision_residues['1'], cmap='hot', interpolation='nearest',
        aspect='auto',
        extent=[min(residue_range), max(residue_range)+1, 0, 0.5],
        norm=matplotlib.colors.Normalize(vmin=min_precision,
                                         vmax=max_precision))
    ax3.vlines(bead_boxes['1'], ymin=0, ymax=0.5, colors='black',
    ...
    # normalize precision over full heatmap. Can you compare precisions
    # of different runs? You cannot but the numerical values need to be
    # compared, so yes. Make it uniform.

    ax4 = fig.add_subplot(gs[4:6, 0])
    ax4.imshow(
        precision_residues['5'], cmap='hot', interpolation='nearest',
        aspect='auto',
        extent=[min(residue_range), max(residue_range)+1, 0, 0.5],
        norm=matplotlib.colors.Normalize(vmin=min_precision,
                                         vmax=max_precision))
    ax4.vlines(bead_boxes['5'], ymin=0, ymax=0.5, colors='black',
               linestyles='dotted', linewidth=1.5)
    ...
    ax5.imshow(
        precision_residues['10'], cmap='hot', interpolation='nearest',
        aspect='auto',
        extent=[min(residue_range), max(residue_range)+1, 0, 0.5],
        norm=matplotlib.colors.Normalize(vmin=min_precision,
                                         vmax=max_precision))
    ax5.vlines(bead_boxes['10'], ymin=0, ymax=0.5, colors='black',
               linestyles='dotted', linewidth=1.5)
    ...
    ax6.imshow(
        precision_residues['20'], cmap='hot', interpolation='nearest',
        aspect='auto',
        extent=[min(residue_range), max(residue_range)+1, 0, 0.5],
        norm=matplotlib.colors.Normalize(vmin=min_precision,
                                         vmax=max_precision))
    ax6.vlines(bead_boxes['20'], ymin=0, ymax=0.5, colors='black',
               linestyles='dotted', linewidth=1.5)
    ...
    ax7.imshow(
        precision_residues['30'], cmap='hot',
        interpolation='nearest', aspect='auto',
        extent=[min(residue_range), max(residue_range)+1, 0, 0.5],
        norm=matplotlib.colors.Normalize(vmin=min_precision,
                                         vmax=max_precision))
    ax7.vlines(bead_boxes['30'], ymin=0, ymax=0.5, colors='black',
               linestyles='dotted', linewidth=1.5)
