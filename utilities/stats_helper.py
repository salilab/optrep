...
def get_sampling_precision(model_sample_id_file, proteins_list, grid_size):
    ...
    for c in cutoffs_list:
        ...
        ctable, retained_clusters = get_contingency_table(
            len(cluster_centers), cluster_members, all_models, run1_all_models,
            run2_all_models)
...
def get_clusters_at_given_precision(model_sample_id_file, proteins_list,
                                    given_precision):
    # get the model index of run1 and run2 models separately
    run1_all_models, run2_all_models = get_run_identity(model_sample_id_file)
    all_models = run1_all_models + run2_all_models
    ...
    cluster_centers, cluster_members = precision_cluster(
        distmat_full, total_num_models, given_precision)
    ...
    for i in range(len(retained_clusters)):
        ...
        print(cluster_centers[clus], file=cluster_centers_file)

        both_file = open('cluster.'+str(i)+'.all.txt', 'w')

        # TODO Not efficient, could have done this while doing the
        # contingency table
            ...
            for component in state.get_children():
                if included_protein(component.get_name(), proteins_list):
                    ...
                    for leaf in IMP.core.get_leaves(component):
                        ...
                        if numModels == 0:
                            frag = IMP.atom.Fragment(leaf)
                            if len(frag.get_residue_indexes()) == 0:
                                weight.append(1.0)
                            else:
                                weight.append(
                                    float(len(frag.get_residue_indexes())))
...
def get_contingency_table(num_clusters, cluster_members, models_subset,
...
class Violations:
    ...
    def get_xlink_distances(self, lndict):
        ...
        for rst in lndict:
            if rst not in self.required_keys:
                continue
...
def get_fit_to_xlinks(xlink_threshold, xlink_keyword):
    ...
    gsm_ids_list = get_run_replica_model_numbers(
        os.path.join('good_scoring_models', 'model_ids_scores.txt'))

    # all other files look the same
    Analysis = Violations(xlink_threshold, xlink_keyword,
                          'run.1/output/stat.0.out')

    distance_between_xlink_beads = {Analysis.required_keys[k]: []
                                    for k in Analysis.required_keys}

    for runid, replicaid, modelid in gsm_ids_list:

        # print(runid,replicaid,modelid)

        # with awk and subprocess, always have awk in one string,
        # the condition and action in one string and the filename in
        # another string, that works!
        stat_file_line = subprocess.check_output(
            ["awk", "NR==" + str(int(modelid) + 2) + " {print} ",
             os.path.join("run." + runid, "output",
                          "stat." + replicaid + ".out")])

        # return a dictionary keyed by crosslinks, and values equal
        # to values for a model
        xlink_distances_model = Analysis.get_xlink_distances(
            eval(stat_file_line.strip('\n')))
    ...
    avg_distances_across_all_xlinks = \
        sum(all_xlinks_all_models_distances) \
        / float(len(all_xlinks_all_models_distances))

    std_err_distances_across_all_xlinks = scipy.stats.sem(
        all_xlinks_all_models_distances)
...
def get_sampling_time(machine_run_on, expt):
    ...
    # on local machine things were returned directly from
    if machine_run_on == "bass":
        sampling_time_file = open(
            os.path.join('average_sampling_time_res' + curr_dir.lstrip('r')
                         + '_expt' + expt + '.txt'), 'r')
        ...
        for sampling_dir in glob.glob(os.path.join('run.*')):
            ...
            curr_job_times = []
            # get the output file for the job, assumes there was only
            # 1 output file.

            job_out_file = glob.glob(os.path.join(sampling_dir, 's*.o[0-9]*'))

            if len(job_out_file) > 1:
                print("Note that there are two sampling output files. "
                      "Manually delete one of them!")
            ...
            start_time = datetime.datetime.strptime(curr_job_times[0],
                                                    date_format)
            end_time = datetime.datetime.strptime(curr_job_times[1],
                                                  date_format)
            ...
            # not available in python versions lower than 2.7
            sampling_times.append(diff.total_seconds())
...
def parse_config_file(config_file):
    ...
    for ln in cf.readlines():
        ...
        if (fields[0] == "RESOLUTIONS_LIST"
                or fields[0] == "PROTEINS_TO_OPTIMIZE_LIST"
                or fields[0] == "DOMAINS_TO_OPTIMIZE_LIST"):
            ...
            if len(fields[1].split()) == 1:  # same linear cutoff for all
                configs_dict[fields[0]] = [
                    fields[1]
                    for i in range(len(configs_dict["RESOLUTIONS_LIST"]))]
            ...
            if fields[1].startswith('~'):  # location of a file or directory
                ...
                # os.path.join did not work here for some weird reason!
                fields[1] = os.path.expanduser('~') + fields[1].lstrip('~')

            # just a single key to string map
            configs_dict[fields[0]] = fields[1]
...
def get_beads_of_xlink_residues(xlink_file, proteins_list):
    ...
    for ln in xf.readlines():
        ...
        if (fields[1] in xlink_beads
                and fields[0] not in xlink_beads[fields[1]]):
            xlink_beads[fields[1]].append(int(fields[0]))

        if (fields[3] in xlink_beads
                and fields[2] not in xlink_beads[fields[3]]):
...
def get_representation_resolution_and_precision(
        proteins_list, domains_list, xlink_file=None):
    # actually we are ignoring the domains list for now, but this could
    # be useful in future

    all_beads_stats = {
        "residues_per_bead": [],
        "diameter_per_bead": [],
        "avg_num_residues": 0.0,
        "avg_dia": 0.0,
        "avg_weighted_dia": 0.0}
    data_beads_stats = {
        "residues_per_bead": [],
        "diameter_per_bead": [],
        "avg_num_residues": 0.0,
    ...
    if xlink_file:
        # return a dictionary by protein
        xlink_beads = get_beads_of_xlink_residues(xlink_file, proteins_list)
        ...
        for component in state.get_children():
            ...
            if prot:
                ...
                for leaf in IMP.core.get_leaves(component):
                    ...
                    # bead with only one residue
                    if len(frag.get_residue_indexes()) == 0:
                        curr_bead_resolution = 1.0

                    else:
                        curr_bead_resolution = float(
                            len(frag.get_residue_indexes()))

                    curr_bead_dia = IMP.core.XYZR(leaf).get_radius() * 2.0

                    all_beads_stats["residues_per_bead"].append(
                        curr_bead_resolution)
                    all_beads_stats["diameter_per_bead"].append(curr_bead_dia)

                    if is_xlink_bead(xlink_beads[prot], leaf):
                        data_beads_stats["residues_per_bead"].append(
                            curr_bead_resolution)
                        data_beads_stats["diameter_per_bead"].append(
                            curr_bead_dia)
                    else:
                        non_data_beads_stats["residues_per_bead"].append(
                            curr_bead_resolution)
                        non_data_beads_stats["diameter_per_bead"].append(
                            curr_bead_dia)

    for dc in [all_beads_stats, data_beads_stats, non_data_beads_stats]:
        dc["avg_num_residues"] = \
            sum(dc["residues_per_bead"]) / float(len(dc["residues_per_bead"]))
        dc["avg_dia"] = \
            sum(dc["diameter_per_bead"]) / float(len(dc["diameter_per_bead"]))
        dc["avg_weighted_dia"] = \
            float(sum([r*d for (r, d) in zip(dc["residues_per_bead"],
                                             dc["diameter_per_bead"])])) \
            / float(sum(dc["residues_per_bead"]))
