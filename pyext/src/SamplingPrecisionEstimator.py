...
class SamplingPrecisionEstimator:
    ...
    ''' Given a set of good-scoring model, estimate the bead-wise sampling
    precision.
    '''

    def __init__(self, gsm_directory, components_calculate_precision=[]):
        """Constructor.
        @param gsm_directory the directory containing good-scoring models.
               Should also contain the sample id file, the file containing
               the sample identity (1 or 2) for each good-scoring model RMF
               (model_sample_ids.txt)
        @param components_calculate_precision is a list of (protein,domain)
               tuples. In future it could be a PMI selection tuple,
               e.g. with keys as groupname, values are tuples for aligning the
               structures e.g. {"Rpb1": (20,100,"Rpb1"),"Rpb2":"Rpb2"}
        """
        ...
        # This dictionary is indexed by a protein domain. The elements are
        # lists of lists.
        # Each list corresponds to a bead and is the list of all coordinates
        # of good-scoring models corresponding to that bead.
        self.bead_coords = {}

        '''Initialize bead precision and diameter dictionaries.'''
        # dictionary with key as protein domain and bead precision for
        # each primitive in the protein domain
        self.bead_precisions = {}
        # dictionary with key as protein domain and size of each primitive
        # in the protein domain
        self.bead_diameter = {}

        # dictionary with key as protein domain and values as booleans
        # that say which primitive is impreicsely sampled
        self.bead_imprecise = {}
        ...
        self.get_models_by_sample(
            sample_id_file=os.path.join(gsm_directory,
                                        "model_sample_ids.txt"))
    ...
    def get_models_by_sample(self, sample_id_file):
        ...
        self.total_num_models = \
            len(self.models_by_sample[1])+len(self.models_by_sample[2])

        sif.close()

    def _included_protein_domain(self, chain_full_name, protein_domain_list):
        ''' Check if the current chain should be included for calculating
        bead precisions, based on the list of protein domains mentioned.
        '''
    ...
    def load_coordinates_and_bead_sizes_from_model_files(self):
        ...
        TODO  There may be memory problem for large assemblies and large number
        of models. Need to refactor.
        TODO Make the code faster by integrating with the GPU based RMSD
        calculator code.
        '''

        # the models in the good_scoring_model dir are called 0.rmf3,
        # 1.rmf3 and so on..
        for i in range(self.total_num_models):
            m = IMP.Model()
            ...
            for state_i in hier_i.get_children():
                for chain_i in state_i.get_children():
                    protein_domain_key = self._included_protein_domain(
                        chain_i.get_name(),
                        self.components_calculate_precision)
                    bead_index = 0
                    ...
                    for a in IMP.atom.get_leaves(chain_i):
                        ...
                        # first model, need to initialize the coordinate and
                        # radius lists for each bead index
                        if i == 0:
                            self.bead_coords[protein_domain_key].append(
                                [curr_coords])
                            # assuming it is same for all models. In future
                            # versions this could be different for each model
                            self.bead_diameter[protein_domain_key].append(
                                curr_dia)
                        else:
                            self.bead_coords[
                                protein_domain_key][bead_index].append(
                                    curr_coords)
    ...
    def get_all_vs_all_distances(self, protein_domain_key, bead_index):
        ...
        # get the all vs all distance matrix, and minimum and maximum
        # distances, of the bead index
        ...
        for i in range(self.total_num_models-1):
            for j in range(i+1, self.total_num_models):
                dist = IMP.algebra.get_distance(
                    self.bead_coords[protein_domain_key][bead_index][i],
                    self.bead_coords[protein_domain_key][bead_index][j])
                distmat[i][j] = dist
    ...
    def get_sampling_precision(self, cutoffs, pvals, cramersv, populations):
        ''' Given the 3 criteria for each cutoff, returns the sampling
        precision.
        ...
        for i in range(len(cutoffs)):
            if populations[i] > 80.0 and (pvals[i] > 0.05
                                          or cramersv[i] < 0.10):
                if sampling_precision > cutoffs[i]:
                    sampling_precision = cutoffs[i]
            else:
                # takes care of local minima? We want to choose the first
                # time at which the above condition was satisfied
                sampling_precision = 100000.0

        return sampling_precision

    def precision_cluster(self, distmat, rmsd_cutoff):
        ''' Perform distance threshold-based clustering given the distance
        matrix and RMSD cutoff        '''
    ...
    def get_contingency_table(self, cluster_members, sample1_models,
                              sample2_models):
        ''' Given the clustering and the identity of models in run1 and run2
        creates the contingency table with 1 row per cluster and 1 column
        per sample.
        '''
    ...
    def percent_ensemble_explained(self, ctable, total_num_models):
        ...
        if len(ctable) == 0:
            return 0.0
        percent_clustered = float(
            numpy.sum(ctable, axis=0)[0]
            + numpy.sum(ctable, axis=0)[1]) * 100.0 / float(total_num_models)

        return percent_clustered

    def test_sampling_exhaustiveness(self, contingency_table,
                                     total_num_models):
    ...
    def estimate_single_bead_precision(self, protein_domain_key, bead_index,
                                       grid_size):
        ''' Estimate the sampling precision of the bead_index-th bead
        of protein domain.
        '''

        (distmat_bead, mindist_bead, maxdist_bead) \
            = self.get_all_vs_all_distances(protein_domain_key, bead_index)

        # the minimum distance is different for different beads,
        # so standardizing it
        cutoffs = numpy.arange(0.0, maxdist_bead, grid_size)
        ...
        for c in cutoffs:
            cluster_centers, cluster_members = self.precision_cluster(
                distmat_bead, c)

            # TODO note that some of these are class members. But I am
            # keeping this general and passing them as arguments for the
            # case where we cluster on subsets of models
            ctable, retained_clusters = self.get_contingency_table(
                cluster_members, self.models_by_sample[1],
                self.models_by_sample[2])

            (pval, crv) = self.test_sampling_exhaustiveness(
                ctable, self.total_num_models)

            percent_explained = self.percent_ensemble_explained(
                ctable, self.total_num_models)
        ...
        sampling_precision = self.get_sampling_precision(
            cutoffs, pvals, cramersv, populations)

        return sampling_precision

    def is_commensurate(self, bead_diameter, bead_precision, xscale):
        ''' Check if the sampling precision of the bead is at most
        xscale times the bead_diameter.
        If not the bead is imprecise and needs to be coarse-grained.
        '''
        linear_cutoff = 2.0
        if bead_precision > xscale*bead_diameter + linear_cutoff:
            # not just larger, but significantly larger than the
            # representation precision
            return False
        return True

    def get_imprecise_beads(self, xscale):
        ''' For each bead check if its size is commensurate with the
        sampling precision. If not, mark it as imprecise.
        @param xscale used to define imprecise bead. imprecise bead
        has sampling_precision > xscale*bead_radius.
        '''

        for protein_domain_key in self.bead_diameter:
            for bead_index in range(
                    len(self.bead_diameter[protein_domain_key])):
                if not self.is_commensurate(
                        self.bead_diameter[protein_domain_key][bead_index],
                        self.bead_precisions[protein_domain_key][bead_index],
                        xscale):
    ...
    def print_bead_precisions(self, out_file_name):
        ''' write bead index, bead precision and whether it is an
        imprecise bead to a file. '''

        with open(out_file_name, 'w') as out_file:
            for protein_domain_key in self.bead_precisions:
                for bead_index in range(
                        len(self.bead_precisions[protein_domain_key])):
                    print(protein_domain_key[0],  protein_domain_key[1],
                          bead_index, "%.2f" % self.bead_precisions[
                              protein_domain_key][bead_index],
                          int(self.bead_imprecise[
                              protein_domain_key][bead_index]),
                          file=out_file)

    def estimate_perbead_sampling_precision(self, grid_size=1.0):
        ''' For each required bead (selection residues mentioned in the
        class constructor), computes the sampling precision.
        ...
        for protein_domain_key in self.components_calculate_precision:
            for bead_index in range(
                    len(self.bead_diameter[protein_domain_key])):
                self.bead_precisions[protein_domain_key].append(
                    self.estimate_single_bead_precision(protein_domain_key,
                                                        bead_index, grid_size))
