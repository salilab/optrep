...
def add_representation(state, input_dir, fasta_file, topology_file,
                       bead_map_file):
    ...
    for ln in tf.readlines():
        ...
        (protein, domain, protein_chain, fastakey, startres_domain,
         endres_domain, pdb, pdb_chain, resolution, clr) = ln.strip().split()

        # TODO not considered multiple copies yet
        if protein not in state.get_molecules():

            if fastakey != "-" and protein_chain != "-":
                curr_mol = state.create_molecule(
                    protein, chain_id=protein_chain,
                    sequence=sequences[fastakey])
                ...
                curr_mol = state.create_molecule(
                    protein, sequence=sequences[fastakey])
        ...
        if pdb != "-":
            ...
            atomic = curr_mol.add_structure(
                os.path.join(input_dir, pdb), chain_id=pdb_chain,
                res_range=(int(startres_domain), int(endres_domain)+1),
                soft_check=True)

        # structure exists, or need not be optimized. Used fixed
        # res representation
        if resolution != "bm":
            multiscale_res = [int(rsn) for rsn in resolution.split(',')]

            curr_mol.add_representation(
                atomic, resolutions=multiscale_res, color=clr)
        ...
        # else add the representation from bead map
            ...
            for bead in bead_map[(protein, domain)]:
                ...
                if pdb != "-":
                    select_residues = \
                        curr_mol[startres_bead-1:endres_bead] & atomic
                else:
                    select_residues = curr_mol[startres_bead-1:endres_bead]

                curr_mol.add_representation(
                    select_residues, resolutions=[endres_bead-startres_bead+1],
                    color=clr)
