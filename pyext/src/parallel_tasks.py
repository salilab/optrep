...
# this file contains the setup and tasks for calculating per-bead
# sampling precision.
...
def master_setup(components_to_update, run_dir, topology_file):
    """
    Given a set of protein/domain components, find the total number
    of beads across proteins.
    """
...
class SlaveTask:
    ...
    def __init__(self, components_to_update, topology_file, run_dir, grid_size,
                 xscale, linear_cutoff, start_bead_index, end_bead_index):
    ...
    def __call__(self):
        ...
        """
        Given a set of protein/domain components, load all their coordinates
        from models into memory.
        Use slaves in a parallel environment to get the sampling precision
        of each bead.
        """
        self.components_to_update = IMP.optrep.ProteinDomainList(
            self.components_to_update)

        spe = IMP.optrep.SPE(
            self.topology_file, self.run_dir, self.components_to_update)

        spe.load_coordinates_and_bead_sizes_from_model_files()

        bead_precision_output_list = spe.print_precision_for_range_of_beads(
            self.start_bead_index, self.end_bead_index,
