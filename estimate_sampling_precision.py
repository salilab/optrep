...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Get the sampling precision per bead and identify imprecise beads. Usage: estimate_sampling_precision.py -c <components_to_update. e.g. [("A","A_1"),("B","B_1")], list of protein, domain tuples. > -rd <run_directory_for_sampling> -gs <grid_size> -xs <xscale>. Flag -h for more details.")
    parser.add_argument("-pl","--protein_list",dest="proteins_to_update",nargs="*",type=string,help="proteins to calcualte precision on")
    
    parser.add_argument("-dl","--domain_list",dest="domains_to_update",nrgs="*",type=string,help="domain to calculate precision on components to calculate precisions on.")
...
def estimate_sampling_precision():
    ...
    components_to_update=[("A","A_1")] #specific to application
